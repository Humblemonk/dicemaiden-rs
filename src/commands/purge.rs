...
use serenity::{
    ...
    prelude::Context,
    all::{CommandInteraction, CommandOptionType, CommandDataOptionValue, GetMessages},
};
...
pub async fn run(
    ...
    let has_permission = match command.member.as_ref() {
        Some(member) => {
            let _guild_id = command.guild_id.ok_or_else(|| {
                anyhow::anyhow!("Purge command can only be used in servers")
            })?;
    ...
    // Get recent messages
    let messages = command.channel_id
    ...
    let deleted_count = if messages.len() == 1 {
        ...
        command.channel_id.delete_message(&ctx.http, messages[0].id).await?;
        1
    } else {
        ...
        match command.channel_id.delete_messages(&ctx.http, &message_ids).await {
            ...
            Err(_) => {
                ...
                for message in &messages {
                    if command.channel_id.delete_message(&ctx.http, message.id).await.is_ok() {
                        deleted += 1;
                    }
                }
                deleted
    ...
    Ok(format!("🗑️ Successfully deleted {} message(s).", deleted_count))
}
