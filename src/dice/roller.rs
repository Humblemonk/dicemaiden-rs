use super::{DiceRoll, Modifier, RollResult, DiceGroup};
    ...
    for modifier in &dice.modifiers {
        match modifier {
            ...
            Modifier::SubtractDice(dice_to_subtract) => {
                ...
                result.individual_rolls.extend(subtract_result.individual_rolls.clone());
...
fn count_wrath_glory_successes(result: &mut RollResult, difficulty: Option<u32>, use_total: bool) -> Result<()> {
    ...
    if use_total {
        ...
        if let Some(dn) = difficulty {
            ...
            result.notes.push(format!("Difficulty {}: {} (needed {}, rolled {})", dn, status, dn, result.total));
        }
        
        // Add notes for wrath die effects (only complications for soak rolls)
        if has_complication {
            result.notes.push("Wrath die rolled 1 - Complication!".to_string());
        ...
        if let Some(dn) = difficulty {
            ...
            let status = if passed { "PASS" } else { "FAIL" };
            result.notes.push(format!("Difficulty {}: {} (needed {})", dn, status, dn));
        }
        ...
        if has_critical {
            result.notes.push("Wrath die rolled 6 - Critical/Glory!".to_string());
        }
...
fn explode_dice(
    ...
    if explosion_count >= max_explosions && indefinite {
        result.notes.push("Maximum explosions reached (100)".to_string());
    }
    ...
    if explosion_count > 0 {
        ...
        if dice_sides == 10 && explode_on == 10 && indefinite {
            ...
            if explosion_count == 1 {
                result.notes.push("⚔️ **RIGHTEOUS FURY!** Natural 10 rolled - additional damage!".to_string());
            } else {
                result.notes.push(format!("⚔️ **RIGHTEOUS FURY!** {} natural 10s - Emperor's wrath unleashed!", explosion_count));
            }
        } else {
            // Generic exploding dice message for other systems
            result.notes.push(format!("{} dice exploded", explosion_count));
        }
    }
...
fn drop_dice(result: &mut RollResult, count: usize) -> Result<()> {
    if count >= result.individual_rolls.len() {
        result.notes.push("Cannot drop more dice than rolled".to_string());
        return Ok(());
    }
...
fn keep_dice(result: &mut RollResult, count: usize, keep_low: bool) -> Result<()> {
    ...
    let mut indexed_rolls: Vec<(usize, i32)> = result.individual_rolls
        .iter()
        .enumerate()
        .map(|(i, &roll)| (i, roll))
        .collect();
    ...
    let kept_indices: Vec<usize> = indexed_rolls.iter()
        .take(count)
        .map(|&(i, _)| i)
        .collect();
...
fn reroll_dice(
    ...
    for i in 0..result.individual_rolls.len() {
        ...
        while result.individual_rolls[i] <= threshold as i32 
            ...
            if rerolls_for_this_die == 1 {
                result.notes.push(format!("Rerolled {} → {}", old_roll, result.individual_rolls[i]));
            }
    ...
    if total_rerolls >= max_total_rerolls {
        result.notes.push("Maximum rerolls reached (100)".to_string());
    }
    
    if total_rerolls > 0 && total_rerolls < 10 { // Don't spam notes for lots of rerolls
        result.notes.push(format!("{} dice rerolled", total_rerolls));
    }
...
fn count_successes(result: &mut RollResult, target: u32) -> Result<()> {
    let successes = result.individual_rolls.iter()
...
fn count_failures(result: &mut RollResult, threshold: u32) -> Result<()> {
    let failures = result.individual_rolls.iter()
...
fn count_botches(result: &mut RollResult, threshold: u32) -> Result<()> {
    let botches = result.individual_rolls.iter()
        .filter(|&&roll| roll <= threshold as i32)
        .count() as i32;
    ...
    if botches > 0 {
        result.notes.push(format!("{} dice botched (≤{})", botches, threshold));
    }
